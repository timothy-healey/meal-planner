...
describe('DayCard', () => {
  it('renders day name', () => {
    const { getByText } = render(<DayCard day={DAY} />);
  ...
  it('renders B / L / D prefix letters', () => {
    const { getAllByText } = render(<DayCard day={DAY} />);
  ...
  it('calls onPress when tapped and onPress is provided', () => {
    ...
    const { getByRole } = render(
      <DayCard day={DAY} onPress={onPress} />
    );
  ...
  it('does not render as a button when onPress is absent', () => {
    const { queryByRole } = render(<DayCard day={DAY} />);
